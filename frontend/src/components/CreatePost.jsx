...
const CreatePost = ({openCreatePost, setOpenCreatePost}) => {
  ...
  const handleCreatePost = async(e) => {
    ...
    try {
      setLoading(true)
      const response = await axios.post('http://localhost:3000/api/v1/post/addpost', formData, {withCredentials: true, headers: {"Content-Type":'multipart/form-data'}});
  ...
  return (
    <Dialog open={openCreatePost}>
      <DialogContent onInteractOutside={()=>setOpenCreatePost(false)}>
        ...
        <Textarea value={caption} onChange={(e)=>setCaption(e.target.value)} className='focus-visible:ring-transparent border-none' placeholder="What's on your mind?"/>
        {
          imagePreview && (
        ...
        <Button onClick={()=>imageRef.current.click()} className='w-fit mx-auto bg-[#0095f6] hover:bg-[#258bcf] '>Select from computer</Button>
        {
          imagePreview && (
            loading ? (
              ...
              <Button onClick={handleCreatePost} type='submit' className='w-full'>Post</Button>
            )
          )
        }
      </DialogContent>
